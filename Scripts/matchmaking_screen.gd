...
func fetch_latest_my_match() -> void:
	...
	if res is Array and res.size() >= 4:
		var code = res[1]
...
func check_if_someone_joined() -> void:
	...
	if response is Array and response.size() >= 4:
		...
		if code == 200:
			...
			if json.parse(body) == OK:
				var data = json.get_data()
				if data is Array and not data.is_empty() and data[0].get("status") == "in_progress":
					match_started = true
					if search_timer: search_timer.stop()
					if poll_timer: poll_timer.stop()
